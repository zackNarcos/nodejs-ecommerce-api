...
exports.logIn = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).header("admin-token", token).send({ "admin-token": token, adminId: foundAdmin._id });
  } catch (error) {
    return res.status(400).send(error);
  }
};
...
exports.getMe = async (req, res) => {
  try {
    ...
    if (!admin) return res.status(400).send({ message: "Admin non trouvé" });
    return res.status(200).send(admin);
  } catch (error) {
    return res.status(400).send({ error: "Une erreur s'est produite, impossible de trouver l'admin" });
  }
}
